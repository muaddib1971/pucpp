...
int eventLog_increment(struct EventLog *e) {
    e->count++;
    return e->count;
}

void eventLog_set(struct EventLog *e, int value) { e->count = value; }

void eventLog_print(struct EventLog *e) { printf("%d\n", e->count); }

// This function acts as a constructor, creating and returning a new "object."
struct EventLog *createEventLog() {
    struct EventLog *newEventLog = (struct EventLog *)malloc(sizeof(EventLog));
    newEventLog->count = 0;
